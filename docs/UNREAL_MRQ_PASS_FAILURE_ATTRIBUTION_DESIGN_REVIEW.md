...
F6  OPEN PRECONDITION for the live gate: arranging a deterministically failing queue-mate without queue mutation,
    a new transport operation, or fixture damage (three candidate mechanisms listed in §5).
...
healthy verdict our own job already recorded. It is an availability/terminal-contract precision defect in the
...
`MovieRenderPipelineEditor/Private/MoviePipelinePIEExecutor.cpp`); (b) this repository's transport and planning
