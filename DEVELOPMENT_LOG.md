...
## September 21, 2026 — Unreal M7 S1 Case-B durable-witness adoption (end-of-night checkpoint)

- Pinned base / current main: `748982713fb6042d875890cbd9999a3bbcbfb3aa`, tree `b851438bdc1d7a03507b72776fa2116b94b5092e`.
- Exact-main arm gate re-established **CLEAR** with 10/10 fail-closed SHA/tree checkpoints, Phase A 11 gates with zero engine RPC, Phase B capability negotiation, and a UE 5.6 build reported up to date with identical DLL hashes.
- **S1 submitted exactly once**: engine accepted the submission, durable engine journal written, lifecycle `FINISHED`, `success=true`, **24 PNGs** (16,170 B each, 388,080 B total), **24/24 manifest hash + size match**, and production-path verification (`verify_render_job_evidence(..., evidence_source_class="ENGINE_JOURNAL_ATTESTED")`) returned `verified = true`.
- **Adoption blocked — F-S1-1**: the engine is simultaneously the liveness bearer and a member of the Job Object whose quiescence the adoption requires. Reconcile returned **Case K** (`WAITING_FOR_ENGINE_QUIESCENCE`, no receipt) while the engine lived and `WAITING_FOR_ENGINE` after it drained. **No receipt exists for the S1 job.** F-S1-2 confirmed: the M9 harness modelled the impossible contained state `engine_capable=True` + `quiescent=True`.
- **Design gate — `DESIGN GATE CLEAR — CASE B CONTRACT DEFINED`**: OPTION A (worker-scoped quiescence) **REJECTED** (would be vacuous under §9); OPTION B (durable / journal-attested Case-B adoption over the prior-session §10 journal) is the target contract. §9 is not weakened — `ActiveProcesses == 0` is established before any witness acquisition or terminal-artifact inspection.
- **Implementation rung**: durable-witness reader (`planning/unreal_witness_journal.py`), quiescence-first ordering and durable binding in the coordinator, F-S1-2 containment model in the harness, plus new M9 suites and the design brief — all as **UNCOMMITTED, UNPUSHED** working-tree changes on `fix/unreal-m7-case-b-durable-witness`.
- Rehearsal on copies of the frozen S1 evidence reached **Case B with exactly one receipt and zero engine RPCs**, leaving the frozen store/journal/artifacts unchanged.
- **Two independent third-party review rounds** (separate Hermes processes, different models, no access to the author's context) returned **THIRD-PARTY BLOCKED** on that revision: B1 (an unreadable journal aborted the whole reconcile pass), B2 (a stray/retained/badly-named journal misrouted a healthy engine-attached job from Case A to Case J), B3 (contradictory `job_id`/`unreal_job_id` was adoptable), B4 (a durable terminal FAILED witness was not adjudicated offline), E3 (documentation contradictions). Both reviewers' findings were reproduced by the author before remediation; the author's own earlier CLEAR verdicts were treated as overturned.
- **Remediation rung (B1–B4 + E3) COMPLETE**: unreadable/malformed witnesses are classified (`PARTIAL` → Case J) and can no longer abort a reconcile pass; candidates are attributed by the §10 logical key so unrelated/retained/badly-named files are ignored and a healthy live job keeps Case A; engine identity must be single and consistent (`unreal_job_id` authoritative for journals, no precedence for contradictions); a durable terminal witness — FINISHED **or** FAILED — is adjudicated offline through the shared adoption tail; runbook/hardening/design docs corrected (no journal deletion while a job is non-terminal).
- Validation: `tests/m7 tests/m8 tests/m9 tests/m10` → **266 passed / 0 failed / 0 skipped / 2.84 s**; `pytest -q -m "not integration"` → **4350 passed / 0 failed / 136 skipped / 13.07 s**. No Blender-gated tests run.
- **Live-path differential vs the pinned base coordinator: 12 scenarios with `journal_root` unset → 0 decision-level deltas**, including both previously discovered accidental semantic changes (live candidate with a different `authorization_id`; live candidate carrying only `unreal_job_id`) now restored to the pinned-base behavior; 4 scenarios with `journal_root` set and no relevant witness → identical to base.
...
## 2026-09-22 — M12.5 architecture-remediation round (invariant commitment, expectation authority, observation identity root)

Context: a DeepSeek/Hermes blind review of PR #137 at `832ad0e0eda08c4113761e28e7222062b284d82c` returned **BLOCKED** with three required-change findings and four non-blocking/informational items. The remediation below was performed by the same agent family as earlier rounds and is therefore **not** a third-party gate; no gate verdict is claimed here, and the next step is a fresh exact-head independent review. The changes below are **committed to the PR #137 branch** as `39633cdd4c55e9fcc3171ff5b590532bdf66f7d0` (docs-only, NOT merged; `832ad0e0` is superseded); PR #137 remains OPEN / DRAFT / NOT MERGED and M12.5 implementation remains unauthorized.

Design changes (`docs/UNREAL_M12_5_SEMANTIC_EVIDENCE_VERIFICATION_V1_DESIGN.md`, v1.1; documentation only — no production code, no M12.1–M12.4 changes):

- **Required-invariant set (F-1, blocker).** §7.1 now defines the set as EXACTLY the resolved source task's `target_state.invariant_names`, binds it to `compute_source_content_digest(source_task) == plan.source_content_digest`, and requires **set equality — not overlap — with the plan's per-step `verification_requirements` union**, failing closed in both directions (`INCOMPLETE_REQUIRED_INVARIANT_SET`, `EXTRA_PLAN_VERIFICATION_REQUIREMENT`). M12.5 establishes this itself; the M12.4 mapping stays optional and is not a substitute. Reproduced defect: source declaring `[scene_initialized, extra_declared_invariant]` with `dependencies=("scene_setup",)` yields a valid M12.3 plan whose requirement union is `[scene_initialized]` only, and only M12.4's factory rejected the pair.
- **Authoritative expected values (F-2, major).** §7.2 + §8.0 make a digest-bound authoritative expectation a registration condition (source, exact field/path, binding mechanism, committed status, unavailable-value behaviour); caller-supplied expectations are inadmissible, and the admissible sources are limited to the resolved source-task commitment and M12.2 registry fields whose commitment lives **outside** the verification request. §8.0.1 records the resulting **v1 coverage limitation** per invariant class: with today's contracts (invariant names only, free-form catalog parameters, no requested entity-ID set, no expected values) no non-render invariant is admissible; the render-evidence requirement is. Consequence recorded honestly: **M12.5 v1 cannot report SATISFIED for a non-render semantic task** until an upstream digest-bound expectation source exists (§26 Q8, out of scope).
- **Observation identity root (F-3, major).** §5.3 roots `request_identity`/`session_identity`/`scope_identity`/`engine_identity`/`extractor_identity`/`contract_revision` in the State Extraction transport response envelope, correlated via `validate_response_correlation`; hand-copied envelope values are untrusted claims and fail closed (`OBSERVATION_IDENTITY_NOT_TRANSPORT_ROOTED`). The recorded limitation is explicit: the extraction path has no authenticated session identity, so v1 proves transport-rooted correlation, **not** authenticated anti-replay freshness (§5.3, §25, Phase C). §5.4 defines the required observation set and the exclusion rule; §5.5 classifies scope divergence (`OBSERVATION_SCOPE_DIVERGENCE`).
- **Render-bearingness (F-4, non-blocking).** §7.3/§9 bind render classification to the digest-bound source and fail closed on a disagreeing plan flag (`PLAN_RENDER_CLASSIFICATION_MISMATCH`); reproduced that `dataclasses.replace(plan, render_plan=False)` keeps `plan_id` and `source_content_digest` unchanged.
- **Result-side identity/provenance tiers (F-5).** §10/§11/§13 close the result shape, enumerate the producer-owned nested structures (`observation_identity`, `render_evidence_identity`, `evidence_trust_basis`, the §13 provenance structure) on the declared tier, and require key **and** value screening on untrusted surfaces; the §11 result allowlist now matches §10's shape exactly.
- **Reuse / authority surface (F-6).** §23.1–§23.3 enumerate permitted read-only surfaces (M12 contracts, frozen extraction boundary, transport contract, M5 evidence verifier, authoritative job/attempt identity records) and forbid submission, receipt store/module, issuance, recovery coordinator, scheduler, transport server and process lifecycle; §12 requires ordinary/aliased/deferred-`importlib`/string-constant detection with four positive controls and a non-vacuity assertion, noting the M7 helper cannot see the deferred form and the existing M12 package-walk gate is a tautology.
- **Informational closure (F-7).** §10.3 defines the immutability mechanism (derived serialization, sealed type, never reading a stored security field — demonstrated that `object.__setattr__` defeats `frozen=True` on `UnrealEvidence`); §20 Phase D states the render-composition determinism criterion; §7/§11 reconcile duplicate-observation wording (`conflicting duplicate` only; identical repeats are not a failure).
- §20 Phase A/C add the mandated adversarial controls; §21 adds the corresponding acceptance criteria; §25 adds the new non-claims; §22 adds review questions 17–23; §26 marks Q2/Q5/Q7 closed and adds Q8 (upstream expectation surface, out of scope).
- **Pre-landing cleanup round (R-3 / R-4 / R-6, plus handoff wording D-1 / D-2 / D-3).** §8.0's third admissibility limb was removed: v1 admits only the resolved source-task commitment and M12.2 registry fields, and it now explicitly forbids `caller request → expected value → digest computed from the same caller request → verification`, with §8.0.1 carrying an admissibility summary naming the inadmissible classes. §5.3 and §6.2 state `transport-rooted ≠ authenticated` and pin the precise claim — the observation payload corresponds to the transport response object and the correlated extraction request it was given; it is NOT established independently that the payload came from the current engine session. A closed, derived `evidence_trust_basis` (`TRANSPORT_CORRELATED` / `DURABLE_RECORD_BACKED` / `NOT_ESTABLISHED` / `NOT_APPLICABLE`, §10.4) was added to the result shape, the §11 result allowlist and refusal classes, the §13 provenance list, §17, §21, §22 Q23 and §25, so transport-correlated observation evidence cannot be read as durable-record-backed render evidence. Handoff/README wording records the round as committed to the PR #137 branch (not merged) and as closing design-rule gaps while bounding v1 capability limitations; no surface implies that the expectation authority exists.
