...
namespace
{
    ...
    void SetMaterialVariantName(AActor* Actor, const FString& VariantName)
    {
        ...
        {
            if (Actor->Tags[Index].ToString().StartsWith(MaterialVariantTagPrefix))
...
FAtlasTransportServer::FAtlasTransportServer()
    : Thread(nullptr), bStopRequested(false), PipeHandle(nullptr)
{
}
...
{
#if PLATFORM_WINDOWS
    HANDLE hPipe = CreateNamedPipeA(TCHAR_TO_ANSI(*PipeName), PIPE_ACCESS_DUPLEX,
        PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT, 1,
        MaxMessageSize, MaxMessageSize, 0, nullptr);
...
bool FAtlasTransportServer::WaitForClient()
{
    ...
    if (!bConnected)
    {
        ...
        if (dwError == ERROR_PIPE_CONNECTED) return true;
...
bool FAtlasTransportServer::WriteResponse(const FString& JsonResponse)
{
    ...
    {
        UE_LOG(LogAtlasTransport, Error, TEXT("WriteFile failed with error: %d"), GetLastError());
        return false;
    }
...
bool FAtlasTransportServer::ParseRequest(const FString& JsonString, FTransportRequest& OutRequest)
{
    ...
    if (!JsonObject->TryGetStringField(TEXT("request_id"), OutRequest.RequestId) ||
        ...
        !JsonObject->TryGetStringField(TEXT("authorization_id"), OutRequest.AuthorizationId)) return false;
    ...
    if (JsonObject->TryGetObjectField(TEXT("arguments"), ArgumentsObject)) OutRequest.Arguments = *ArgumentsObject;
    return true;
}
...
FString FAtlasTransportServer::SerializeResponse(const FTransportResponse& Response)
{
    ...
    for (const FString& EntityId : Response.EntityIds) EntityIdsArray.Add(MakeShareable(new FJsonValueString(EntityId)));
    JsonObject->SetArrayField(TEXT("entity_ids"), EntityIdsArray);
    if (Response.ObservedState.IsValid()) JsonObject->SetObjectField(TEXT("observed_state"), Response.ObservedState);
    else JsonObject->SetObjectField(TEXT("observed_state"), MakeShareable(new FJsonObject));
...
bool FAtlasTransportServer::ValidateRequest(const FTransportRequest& Request, FString& OutError)
{
    ...
    {
        if (EntityId.IsEmpty() || EntityId.TrimStartAndEnd().IsEmpty()) { OutError = TEXT("entity_ids cannot contain empty strings"); return false; }
    }
    if (!Request.Arguments.IsValid()) { OutError = TEXT("arguments cannot be null"); return false; }

    TArray<FString> ArgumentEntityIds;
    if (!Request.Arguments->TryGetStringArrayField(TEXT("entity_ids"), ArgumentEntityIds)) { OutError = TEXT("arguments.entity_ids must be an array of strings"); return false; }
    ...
    {
        if (ArgumentEntityIds[Index] != Request.EntityIds[Index]) { OutError = TEXT("arguments.entity_ids must match entity_ids"); return false; }
    }

    if (Request.OperationName == TEXT("inspect_target_actors"))
    {
        if (Request.Capability != TEXT("inspect_actor") || Request.Kind != TEXT("read")) { OutError = TEXT("inspect_target_actors requires inspect_actor/read"); return false; }
        return true;
    }
    ...
    if (Request.OperationName == TEXT("set_actor_location"))
    {
        ...
        if (!Request.Arguments->TryGetObjectField(TEXT("location"), LocationObject) || LocationObject == nullptr || !LocationObject->IsValid()) { OutError = TEXT("arguments.location must be an object"); return false; }
        double X = 0.0, Y = 0.0, Z = 0.0;
        if (!(*LocationObject)->TryGetNumberField(TEXT("x"), X) || !(*LocationObject)->TryGetNumberField(TEXT("y"), Y) || !(*LocationObject)->TryGetNumberField(TEXT("z"), Z)) { OutError = TEXT("arguments.location must contain numeric x, y, and z"); return false; }
        return true;
    }
    ...
    if (Request.OperationName == TEXT("set_actor_rotation"))
    {
        ...
        if (!Request.Arguments->TryGetObjectField(TEXT("rotation"), RotationObject) || RotationObject == nullptr || !RotationObject->IsValid()) { OutError = TEXT("arguments.rotation must be an object"); return false; }
        double Pitch = 0.0, Yaw = 0.0, Roll = 0.0;
        if (!(*RotationObject)->TryGetNumberField(TEXT("pitch"), Pitch) || !(*RotationObject)->TryGetNumberField(TEXT("yaw"), Yaw) || !(*RotationObject)->TryGetNumberField(TEXT("roll"), Roll)) { OutError = TEXT("arguments.rotation must contain numeric pitch, yaw, and roll"); return false; }
        return true;
    }

    if (Request.OperationName == TEXT("inspect_material_state"))
    {
        if (Request.Capability != TEXT("material") || Request.Kind != TEXT("read")) { OutError = TEXT("inspect_material_state requires material/read"); return false; }
        return true;
    }
    ...
    if (Request.OperationName == TEXT("apply_material_variant"))
    {
        ...
        if (!Request.Arguments->TryGetObjectField(TEXT("material_variant"), VariantObject) || VariantObject == nullptr || !VariantObject->IsValid()) { OutError = TEXT("arguments.material_variant must be an object"); return false; }
        FString VariantName;
        if (!(*VariantObject)->TryGetStringField(TEXT("name"), VariantName) || VariantName.TrimStartAndEnd().IsEmpty()) { OutError = TEXT("arguments.material_variant.name must be a non-empty string"); return false; }
        return true;
    }
...
bool FAtlasTransportServer::ExecuteRequest(const FTransportRequest& Request, FTransportResponse& OutResponse)
{
    ...
    if (Request.OperationName == TEXT("inspect_target_actors") ||
        ...
        Request.OperationName == TEXT("set_actor_rotation") ||
        Request.OperationName == TEXT("inspect_material_state") ||
        Request.OperationName == TEXT("apply_material_variant"))
    {
        ...
        const bool bEventTriggered = SharedState->CompletionEvent->Wait(TimeoutMs);
        if (bStopRequested)
        {
...
void FAtlasTransportServer::ExecuteOnGameThread(TSharedPtr<FGameThreadExecutionState> SharedState)
{
    ...
    if (SharedState->Request.OperationName == TEXT("inspect_target_actors"))
        bTaskSuccess = InspectTargetActors(SharedState->Request.EntityIds, SharedState->ObservedState, SharedState->Error);
    else if (SharedState->Request.OperationName == TEXT("set_actor_location"))
        bTaskSuccess = SetActorLocation(SharedState->Request, SharedState->ObservedState, SharedState->Error);
    else if (SharedState->Request.OperationName == TEXT("set_actor_rotation"))
        bTaskSuccess = SetActorRotation(SharedState->Request, SharedState->ObservedState, SharedState->Error);
    else if (SharedState->Request.OperationName == TEXT("inspect_material_state"))
        bTaskSuccess = InspectMaterialState(SharedState->Request.EntityIds, SharedState->ObservedState, SharedState->Error);
    else if (SharedState->Request.OperationName == TEXT("apply_material_variant"))
        bTaskSuccess = ApplyMaterialVariant(SharedState->Request, SharedState->ObservedState, SharedState->Error);
    else
        SharedState->Error = FString::Printf(TEXT("Unsupported operation: %s"), *SharedState->Request.OperationName);
...
bool FAtlasTransportServer::SetActorLocation(const FTransportRequest& Request, TSharedPtr<FJsonObject>& OutObservedState, FString& OutError)
{
    ...
    if (!Actor || !IsValid(Actor)) { OutError = FString::Printf(TEXT("Actor not found for entity_id: %s"), *Request.EntityIds[0]); return false; }
...
bool FAtlasTransportServer::InspectMaterialState(const TArray<FString>& EntityIds, TSharedPtr<FJsonObject>& OutObservedState, FString& OutError)
{
    ...
    for (const FString& EntityId : EntityIds)
    {
        ...
        if (!Actor || !IsValid(Actor)) { OutError = FString::Printf(TEXT("Actor not found for entity_id: %s"), *EntityId); return false; }
        TSharedPtr<FJsonObject> MaterialState;
        if (!BuildMaterialVariantState(Actor, MaterialState, OutError)) return false;
        TSharedPtr<FJsonObject> ActorData = MakeShareable(new FJsonObject);
        ActorData->SetStringField(TEXT("entity_id"), EntityId);
        ActorData->SetObjectField(TEXT("material"), MaterialState);
...
bool FAtlasTransportServer::ApplyMaterialVariant(const FTransportRequest& Request, TSharedPtr<FJsonObject>& OutObservedState, FString& OutError)
{
    ...
    for (const FString& EntityId : Request.EntityIds)
    {
        AActor* Actor = FindActorByEntityId(EntityId);
        if (!Actor || !IsValid(Actor)) { OutError = FString::Printf(TEXT("Actor not found for entity_id: %s"), *EntityId); return false; }
        SetMaterialVariantName(Actor, VariantName);
    }
...
bool FAtlasTransportServer::BuildMaterialVariantState(AActor* Actor, TSharedPtr<FJsonObject>& OutMaterialState, FString& OutError)
{
    ...
    TSharedPtr<FJsonObject> Variant = MakeShareable(new FJsonObject);
    Variant->SetStringField(TEXT("name"), GetMaterialVariantName(Actor));
    OutMaterialState = MakeShareable(new FJsonObject);
    OutMaterialState->SetObjectField(TEXT("variant"), Variant);
    return true;
}
...
bool FAtlasTransportServer::InspectTargetActors(const TArray<FString>& EntityIds, TSharedPtr<FJsonObject>& OutObservedState, FString& OutError)
{
    ...
    if (GEngine->GetWorldContexts().Num() > 0) World = GEngine->GetWorldContexts()[0].World();
...
AActor* FAtlasTransportServer::FindActorByEntityId(const FString& EntityId)
{
    ...
    for (TActorIterator<AActor> ActorItr(World); ActorItr; ++ActorItr)
    {
        ...
        if (Actor && IsValid(Actor) && Actor->Tags.Contains(FName(*TagToFind))) return Actor;
    }
    return nullptr;
}
