...
namespace
{
    ...
    const FName BallFixtureTag(TEXT("atlas_entity:ball"));
    const FName BallFixtureActorName(TEXT("AtlasBallFixture"));
    const FName PlayerFixtureTag(TEXT("atlas_entity:player-09"));
    const FName PlayerFixtureActorName(TEXT("AtlasPlayer09Fixture"));
...
void FAtlasUnrealTransportModule::StartupModule()
{
    ...
    // Initialize Atlas Live streaming subsystem (queue, TCP listener, GameThread pump)
    LiveIngressQueue = MakeShared<FAtlasLiveIngressQueue>(128);

    // Initialize effect registry and register default handlers
    LiveEffectRegistry = MakeShared<FAtlasLiveEffectRegistry>(500.0); // 500ms visual deadline

    // 1. IMPACT_ACCENT handlers
    TSharedPtr<FAtlasLiveImpactAccentHandler> ImpactHandler = MakeShared<FAtlasLiveImpactAccentHandler>();
    LiveEffectRegistry->RegisterHandler(EAtlasLiveTreatment::ImpactAccent, TEXT("strike_flash_v1"), ImpactHandler);
    LiveEffectRegistry->RegisterHandler(EAtlasLiveTreatment::ImpactAccent, TEXT("NS_LiveSoccer_BallStrike_Burst"), ImpactHandler);
    LiveEffectRegistry->RegisterHandler(EAtlasLiveTreatment::ImpactAccent, TEXT(""), ImpactHandler); // Default fallback

    // 2. SPEED_TRAIL handlers
    TSharedPtr<FAtlasLiveSpeedTrailHandler> TrailHandler = MakeShared<FAtlasLiveSpeedTrailHandler>();
    LiveEffectRegistry->RegisterHandler(EAtlasLiveTreatment::SpeedTrail, TEXT("speed_trail_v1"), TrailHandler);
    LiveEffectRegistry->RegisterHandler(EAtlasLiveTreatment::SpeedTrail, TEXT("NS_LiveSoccer_BallTrail_Ribbon"), TrailHandler);
    LiveEffectRegistry->RegisterHandler(EAtlasLiveTreatment::SpeedTrail, TEXT(""), TrailHandler); // Default fallback

    // 3. IMPACT_FRAME handlers
    TSharedPtr<FAtlasLiveImpactFrameHandler> FrameHandler = MakeShared<FAtlasLiveImpactFrameHandler>();
    LiveEffectRegistry->RegisterHandler(EAtlasLiveTreatment::ImpactFrame, TEXT("impact_frame_v1"), FrameHandler);
    LiveEffectRegistry->RegisterHandler(EAtlasLiveTreatment::ImpactFrame, TEXT("PP_LiveSoccer_ImpactFrame"), FrameHandler);
    LiveEffectRegistry->RegisterHandler(EAtlasLiveTreatment::ImpactFrame, TEXT(""), FrameHandler); // Default fallback

    LiveGameThreadPump = MakeShared<FAtlasLiveGameThreadPump>(LiveIngressQueue, LiveEffectRegistry, 16);
    LiveGameThreadPump->Start();

    // Bind Live TCP listener to default port 7778 (fallback dynamic if occupied)
    LiveTcpListener = MakeShared<FAtlasLiveTcpListener>(LiveIngressQueue, 7778, TEXT("127.0.0.1"));
    if (LiveTcpListener->Start())
    {
        UE_LOG(LogAtlasTransport, Log, TEXT("Atlas Live TCP listener started on port %d"), LiveTcpListener->GetBoundPort());
    }
    else
    {
        UE_LOG(LogAtlasTransport, Warning, TEXT("Failed to start Atlas Live TCP listener on 7778, trying dynamic port"));
        LiveTcpListener = MakeShared<FAtlasLiveTcpListener>(LiveIngressQueue, 0, TEXT("127.0.0.1"));
        if (LiveTcpListener->Start())
        {
            UE_LOG(LogAtlasTransport, Log, TEXT("Atlas Live TCP listener started on dynamic port %d"), LiveTcpListener->GetBoundPort());
        }
    }
...
bool FAtlasUnrealTransportModule::EnsureSequencerFixture(float DeltaTime)
{
    ...
    // Create or reuse deterministic Ball fixture actor
    AActor* BallFixtureActor = nullptr;
    for (TActorIterator<AActor> It(World); It; ++It)
    {
        AActor* ExistingActor = *It;
        if (ExistingActor && IsValid(ExistingActor) && ExistingActor->ActorHasTag(BallFixtureTag))
        {
            BallFixtureActor = ExistingActor;
            break;
        }
    }
    if (!BallFixtureActor)
    {
        FActorSpawnParameters SpawnParams;
        SpawnParams.Name = BallFixtureActorName;
        SpawnParams.NameMode = FActorSpawnParameters::ESpawnActorNameMode::Requested;
        SpawnParams.ObjectFlags |= RF_Transient;

        BallFixtureActor = World->SpawnActor<AActor>(
            AActor::StaticClass(),
            FTransform(FVector(100.0f, 0.0f, 15.0f)),
            SpawnParams);

        if (BallFixtureActor)
        {
            USceneComponent* BallRoot = NewObject<USceneComponent>(BallFixtureActor, TEXT("AtlasBallFixtureRoot"));
            if (BallRoot)
            {
                BallFixtureActor->SetRootComponent(BallRoot);
                BallRoot->RegisterComponent();
                BallFixtureActor->Tags.AddUnique(BallFixtureTag);
                BallFixtureActor->SetActorLabel(TEXT("Atlas Ball Fixture"));
            }
        }
    }

    // Create or reuse deterministic Player fixture actor
    AActor* PlayerFixtureActor = nullptr;
    for (TActorIterator<AActor> It(World); It; ++It)
    {
        AActor* ExistingActor = *It;
        if (ExistingActor && IsValid(ExistingActor) && ExistingActor->ActorHasTag(PlayerFixtureTag))
        {
            PlayerFixtureActor = ExistingActor;
            break;
        }
    }
    if (!PlayerFixtureActor)
    {
        FActorSpawnParameters SpawnParams;
        SpawnParams.Name = PlayerFixtureActorName;
        SpawnParams.NameMode = FActorSpawnParameters::ESpawnActorNameMode::Requested;
        SpawnParams.ObjectFlags |= RF_Transient;

        PlayerFixtureActor = World->SpawnActor<AActor>(
            AActor::StaticClass(),
            FTransform(FVector(90.0f, 0.0f, 0.0f)),
            SpawnParams);

        if (PlayerFixtureActor)
        {
            USceneComponent* PlayerRoot = NewObject<USceneComponent>(PlayerFixtureActor, TEXT("AtlasPlayer09FixtureRoot"));
            if (PlayerRoot)
            {
                PlayerFixtureActor->SetRootComponent(PlayerRoot);
                PlayerRoot->RegisterComponent();
                PlayerFixtureActor->Tags.AddUnique(PlayerFixtureTag);
                PlayerFixtureActor->SetActorLabel(TEXT("Atlas Player 09 Fixture"));
            }
        }
    }
...
void FAtlasUnrealTransportModule::ShutdownModule()
{
    ...
    if (LiveGameThreadPump.IsValid())
    {
        LiveGameThreadPump->Stop();
        LiveGameThreadPump.Reset();
    }

    if (LiveEffectRegistry.IsValid())
    {
        LiveEffectRegistry->CleanupAllActiveEffects();
        LiveEffectRegistry.Reset();
    }

    if (LiveTcpListener.IsValid())
    {
        LiveTcpListener->StopListener();
        LiveTcpListener.Reset();
    }

    if (LiveIngressQueue.IsValid())
    {
        LiveIngressQueue->Shutdown();
        LiveIngressQueue.Reset();
    }
