...
- **M12 (Unreal Semantic Soccer Production Layer) is the next Unreal development milestone.** Design (PR #86) merged; **M12.1 implemented** (semantic task contract, normalization, compile boundary) and **M12.2 implemented** (versioned catalog + reusable fragments + composition) — `planning/m12/`; docs `UNREAL_M12_1_SEMANTIC_TASK_CONTRACT.md` + `UNREAL_M12_2_CATALOG_FRAGMENTS_COMPOSITION.md`. Render-bearing execution remains deferred; M12.4 (semantic→existing-runtime adapter) next.
