...
- **M12 (Unreal Semantic Soccer Production Layer) is the next Unreal development milestone.** Design (PR #86) merged; **M12.1** (semantic task contract + normalize/compile), **M12.2** (catalog + fragments + composition), **M12.3** (semantic execution-plan boundary) implemented — `planning/m12/`; docs `UNREAL_M12_1_SEMANTIC_TASK_CONTRACT.md` / `UNREAL_M12_2_CATALOG_FRAGMENTS_COMPOSITION.md` / `UNREAL_M12_3_EXECUTION_PLAN.md`. Render-bearing execution remains deferred; M12.4 (semantic→existing-runtime adapter) next.
