"""Direct live probe for the authorization-bound Blender move write gate."""
import argparse
import json
...
from planning.object_move_task import TARGET_LOCATION, TARGET_OBJECT
from tools.blender import move_object
from tools.blender_transform import inspect_object_transform


FILE_BY_CASE = {
    "incorrect": "object_move_INCORRECT.blend",
    "correct": "object_move_CORRECT.blend",
}
...
def _execute(tool: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
    if tool != "move_object":
        raise RuntimeError("live probe only permits move_object")
    raw = move_object(**arguments)
    status = raw.get("status")
    return {
        "ok": status in {"moved", "already_moved"},
...
def _verifier(action: ActionSpec, receipt: Any) -> Tuple[bool, Dict[str, Any]]:
    ...
    target = [float(value) for value in action.arguments["location"]]
    ...
    return matches, {"authoritative": observed, "location_matches": matches}
...
def main() -> None:
    ...
    authorization = BlenderWriteAuthorization.issue(action, f"live-probe:move:{args.case}")
    ...
    print("ATLAS BLENDER LIVE MOVE WRITE GATE PROBE")
    print(json.dumps({
        "case": args.case,
    ...
    if args.adversarial:
        if outcome.status != "BLOCKED" or outcome.receipt is not None:
            raise SystemExit("ADVERSARIAL MOVE PROBE FAILED: false success escaped the gate")
        print("ATLAS BLENDER LIVE MOVE ADVERSARIAL GATE: PASS")
    else:
        if outcome.status != "VERIFIED" or outcome.receipt is None:
            raise SystemExit("LIVE MOVE PROBE FAILED: authoritative verification did not confirm the write")
        print("ATLAS BLENDER LIVE MOVE VERIFIED: PASS")
