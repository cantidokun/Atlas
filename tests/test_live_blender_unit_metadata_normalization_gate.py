"""LIVE Blender gate for Wave 8 unit metadata normalization.

Operator-gated and skipped unless ATLAS_RUN_LIVE_BLENDER=1. The probe validates the supported
Blender-side unit metadata boundary independently from the canonical executor.
"""
...
SCRIPT = Path(__file__).with_name("unit_metadata_normalization_live_script.py")
...
def test_wave8_live_blender_boundary():
    ...
    marker = next(
        (line for line in proc.stdout.splitlines() if line.startswith("ATLAS_WAVE8_UNIT_METADATA_LIVE_RESULT=")),
        None,
    )
    ...
    assert "ATLAS_WAVE8_UNIT_METADATA_LIVE_PASS" in proc.stdout
