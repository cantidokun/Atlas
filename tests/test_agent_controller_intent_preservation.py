...
def test_admission_preserves_only_canonical_capability_request_fields():
    ...
    classified = host.process_model_response(
        "ATLAS_CONTROLLER_REQUEST: "
        '{"capability":"production","provider":"unreal",'
        '"context":{"production":true},"intent":"model-intent"}'
    )
    ...
    assert isinstance(admission.result, object)
