...
def _operation(task_id, revision, writes):
    checkpoint = ProductionTaskCheckpoint.create(task_id, revision, (), {"task": task_id}, f"auth-{task_id}")
...
def _completed_checkpoint(revision):
    checkpoint = ProductionTaskCheckpoint.create("task-1", revision, (), {"done": True}, "auth-1")
