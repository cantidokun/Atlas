...
def test_blender_factory_rejects_non_blender_engine():
    ...
    evidence = BlenderPersistenceEvidence.create("move_object", arguments, "inspect_scene", inspection.details, inspection.details, inspection)
    with pytest.raises(ProductionArtifactError, match="Blender artifact lineage engine must be Blender"):
        ProductionArtifactManifest.from_blender_closed_loop(artifact_id="artifact-001", canonical_digital_twin_id="twin-001", representation_type="blender-scene", artifact_path="scene.blend", operation_receipt=receipt, persistence_evidence=evidence, engine="Unreal")


def test_unreal_factory_rejects_non_unreal_engine():
    evidence = UnrealEvidence(operation_name="inspect_render_job", entity_ids=("FIELD_SURFACE",), observed_state={"job_id": "job-001", "sequence_asset_path": "/Game/Atlas/Sequence", "status": "completed", "success": True, "failed": False, "output_files": ["C:/renders/AtlasRender_0001.png"]}, verified=True, source="engine-test")
    ...
    with pytest.raises(ProductionArtifactError, match="Unreal artifact lineage engine must be Unreal"):
        ProductionArtifactManifest.from_unreal_render_receipt(artifact_id="artifact-001", canonical_digital_twin_id="twin-001", representation_type="unreal-render", artifact_path="C:/renders/AtlasRender_0001.png", render_receipt=receipt, render_evidence=evidence, engine="Blender")
