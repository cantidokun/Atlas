...
def test_live_blender_duplicate_vertex_removal_gate():
    ...
    script = Path(__file__).with_name("duplicate_vertex_removal_live_script.py")
    ...
    assert "ATLAS_WAVE7_DUPLICATE_VERTEX_LIVE_PASS" in output, output
    ...
    for name in ("material_slots_preserved", "material_slot_count_preserved",
        ...
        assert evidence["checks"].get("safe_" + name) is True, (name, evidence)
    # ANTI-VACUITY: the fixture really carried assigned + unassigned slots before the mutation
    assert evidence["before_slots"] == [["DATA", "wave7_turf"], ["DATA", "wave7_line_markings"],
