...
class FakeNiagaraAdapter(UnrealAdapterProduction):
    ...
    def _evidence(self, operation):
        return UnrealEvidence(operation_name=operation.name, entity_ids=tuple(operation.entity_ids), observed_state={"FIELD_SURFACE": {"niagara": {"variant": {"name": self.variant}}}}, source="fake-unreal", verified=False)


def _intent(intent_id):
    return UnrealTaskIntent(intent_id=intent_id, description="Niagara executor semantic verification", target_entity_ids=("FIELD_SURFACE",))


def test_executor_semantically_verifies_niagara_variant_after_write():
    result = UnrealPlanExecutor(FakeNiagaraAdapter("default")).execute(UnrealTaskPlanner().plan_niagara_variant(_intent("niagara-proof"), {"name": "goal_burst"}), "niagara-proof-auth")
    assert result.success is True
    assert result.evidence_ledger[-1].observed_state["FIELD_SURFACE"]["niagara"]["variant"]["name"] == "goal_burst"
...
def test_executor_fails_when_post_write_niagara_variant_does_not_match_target():
    ...
    with pytest.raises(UnrealPlanExecutionError, match="Niagara variant") as exc_info:
        UnrealPlanExecutor(adapter).execute(UnrealTaskPlanner().plan_niagara_variant(_intent("niagara-fail"), {"name": "goal_burst"}), "niagara-fail-auth")
