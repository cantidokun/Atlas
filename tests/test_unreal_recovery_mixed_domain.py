...
class MixedDomainRecoveryTransport:
    def __init__(self):
        self.state = {
            ENTITY_ID: {
                ...
                "material": {"variant": {"name": "default_surface"}},
    ...
    def send(self, request):
        ...
        if name == "apply_material_variant":
            self.state[ENTITY_ID]["material"] = {"variant": {"name": request.arguments["variant"]}}
            return self._response(request)
        if name == "apply_niagara_variant":
            ...
            self.state[ENTITY_ID]["niagara"] = {"variant": {"name": request.arguments["variant"]}}
            return self._response(request)
    ...
    def _response(self, request, success=True, error=""):
        return UnrealTransportResponse(
            ...
            error=error,
            source="mixed-domain-recovery-transport",
        )


def _plan():
    return UnrealTaskPlan(
        "mixed-domain-recovery",
        (
            UnrealOperation(
            ...
            UnrealOperation(
                ...
                {"entity_ids": (ENTITY_ID,), "variant": "liquid_surface"},
                (ENTITY_ID,),
            ),
            UnrealOperation(
                UnrealCapability.MATERIAL,
                UnrealOperationKind.VERIFY,
                "verify_material_variant",
                {"entity_ids": (ENTITY_ID,), "expected_material_variant": {"name": "liquid_surface"}},
                (ENTITY_ID,),
            ),
            UnrealOperation(
                ...
                {"entity_ids": (ENTITY_ID,), "variant": "goal_burst"},
                (ENTITY_ID,),
            ),
            UnrealOperation(
                UnrealCapability.NIAGARA,
                UnrealOperationKind.VERIFY,
                "verify_niagara_variant",
                {"entity_ids": (ENTITY_ID,), "expected_niagara_variant": {"name": "goal_burst"}},
                (ENTITY_ID,),
            ),
        ),
    )
...
def test_mixed_domain_failure_reassesses_all_prior_domains_and_replaces_only_unresolved_domain():
    ...
    assert [op.name for op in replacement.operations] == [
        "apply_niagara_variant",
        "verify_niagara_variant",
    ]
    ...
    assert [request.operation_name for request in transport.requests[baseline:]] == [
        "inspect_target_actors",
        "inspect_material_state",
        "inspect_niagara_state",
    ]
