...
class UnrealPlanExecutionError(RuntimeError):
    def __init__(self,message,*,failure=None): super().__init__(message); self.failure=failure
...
class UnrealPlanExecutor:
    ...
    def _execute_one(self,operation,authorization_id,expected_location=None,expected_rotation=None,expected_scale=None,expected_material_variant=None,expected_niagara_variant=None):
        arguments=dict(operation.arguments); arguments["entity_ids"]=tuple(operation.entity_ids); arguments["authorization_id"]=authorization_id
        validate_unreal_tool_call(operation.name,arguments)
        method_name=self._DISPATCH.get(operation.kind)
        if method_name is None: raise UnrealPlanExecutionError(f"No adapter endpoint for operation kind '{operation.kind.value}'")
    ...
    def execute_authorized(self,plan,authorization):
        if not isinstance(authorization,UnrealPlanAuthorization): raise TypeError("authorization must be an UnrealPlanAuthorization instance")
    ...
    def execute(self,plan,authorization_id):
        ...
        ledger=[]; completed=[]; expected_location=expected_rotation=expected_scale=expected_material_variant=expected_niagara_variant=None
        for index,operation in enumerate(plan.operations):
            ...
            try:
                evidence=self._execute_one(operation,authorization_id,expected_location if operation.kind is UnrealOperationKind.VERIFY else None,expected_rotation if operation.kind is UnrealOperationKind.VERIFY else None,expected_scale if operation.kind is UnrealOperationKind.VERIFY else None,expected_material_variant if operation.kind is UnrealOperationKind.VERIFY else None,expected_niagara_variant if operation.kind is UnrealOperationKind.VERIFY else None)
