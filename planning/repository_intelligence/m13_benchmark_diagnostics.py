...
def diagnose_case(result: ContextEvaluationResult) -> ContextDiagnostic:
    ...
    required = tuple(sorted(set(result.required_paths) - selected))
    relevant = tuple(sorted(set(result.relevant_paths) - selected))
    extra = tuple(sorted(selected - set(result.relevant_paths)))
