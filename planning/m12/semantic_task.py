...
def compute_source_content_digest(source_task: "UnrealProductionTaskDefinition") -> str:
    ...
    layer. It is generated by M12.3 (:func:`generate_execution_plan`) from the
    authoritative resolved source task and carried immutably on the plan; M12.4
    recomputes it (via the same function) to verify the plan's commitment
    against the supplied source.

    It binds the task identity, version, digital-twin id, target state, dependencies,
    evidence, actions, allowed tools/mutations, and resolved catalog metadata
    (parameters, fragments) via STRICT JSON (``allow_nan=False``, sorted keys,
    compact separators), so two same-identity tasks with different resolved content
    yield different digests and coercion cannot collapse distinct inputs.

    The canonical form is deterministic and stable for semantically equivalent
    source content. Raises ``UnrealSemanticTaskError`` on non-JSON data.
    """
    if not isinstance(source_task, UnrealProductionTaskDefinition):
        raise TypeError(
            "source_task must be an UnrealProductionTaskDefinition"
        )
    try:
