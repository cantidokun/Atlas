...
class MeshModel:
    ...
    Wave 11 permits two truthful zero-face states:
    - vertices present with zero faces (a vertex-only Blender mesh);
    - zero vertices and zero faces (the exact canonical result of removing every isolated vertex).
    A non-empty face set always requires vertices and valid indices.
    """
...
class ObjectModel:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "rotation", tuple(_exact_finite_number(v, "object.rotation") for v in self.rotation))
...
def parse_scene_report_input(value: Any) -> SceneModel:
    ...
    return SceneModel(scene_id=root.get("scene_id", ""), unit_system=root.get("unit_system", ""), objects=tuple(_parse_object(o) for o in objects_raw), coordinate_frame=root.get("coordinate_frame"), world_bounds=root.get("world_bounds"))
