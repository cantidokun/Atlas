...
11. Any expected-postcondition identifier or digest carried by bridge transport is binding metadata only. The bridge must never accept caller-supplied postcondition text, predicates, or digests as the authority for success.
...
These identifiers are transport-level aliases for the existing canonical correction type identifiers; they must not become a second correction taxonomy.

The bridge allowlist is explicitly implementation-owned and closed to these four operations. It must validate that each identifier matches the corresponding canonical executor/mapping identifier. A future correction added to the generic or correction system must NOT become bridge-executable merely because it appears in a broader executor allowlist; it requires an explicit bridge implementation and new live evidence.
...
Optional transport metadata may include bridge_version, correction_executor_version, and source_scene_id. These are transport provenance only: they must not select acceptance criteria, alter canonical bindings, or participate in postcondition digest authority beyond the bridge's own fixed contract/version binding.

The last two postcondition fields are **not authority inputs**. They are transport bindings generated by the bridge from the already-selected canonical operation, executor version, and canonical postcondition contract. A caller must not be able to choose a postcondition definition or digest and thereby weaken acceptance criteria. The bridge's sole acceptance path is its own canonical computation; a mismatch is a typed transport failure used only to detect tampering. These fields must never participate in selecting or defining which postconditions run. The implementation must derive the binding from the existing executor contract rather than inventing a parallel digest helper with independently drifting semantics.

The canonical correction plan remains the source of truth; the bridge request is a binding envelope, not a second plan.
...
When a real source .blend is supplied, it may be loaded into the bounded background process for the correction invocation, but no save/write command is permitted. When the live gate uses disposable in-memory fixtures, the same lifecycle applies without a source file. In both cases the authoritative post-state comes from the same in-memory Blender session in which the mutation occurred.
...
The canonical correction executor's gate order is authoritative. The following is the bridge lifecycle-level ordering and must not be interpreted as a competing executor pipeline.

1. Receive a closed correction request.
2. Validate envelope shape and the closed operation vocabulary.
3. Canonically compute/verify the expected postcondition binding. Caller-controlled predicate definitions are rejected; a mismatch is a typed transport failure before engine contact.
4. For W2/W3, complete the existing correction authorization gate **before any Blender extraction or mutation**. An authorization rejection consumes zero engine/extractor contact. W1/W1b have no authorization-artifact gate and continue directly to session creation.
5. Open the bounded Blender session only after any required authorization gate has passed.
6. Perform authoritative source binding by extracting the pre-state/report from that same bounded session and checking the plan's source digest.
7. Run the existing correction preconditions on the fresh authoritative pre-state.
...
12. Run the existing canonical postconditions.
...
The bridge must not introduce a second authorization gate. Structural transport validation is permitted, but it is not authority. On every authorization-rejection path the session must either not be created at all or, if the transport implementation must instantiate a process container before dispatch, it must not load a scene, perform extraction, or mutate engine state.
...
- a real source .blend may be loaded for the bounded session;
...
Live evidence should demonstrate no-save with source/output path state, created-file inventory including .blend/.blend1 and the isolated session's autosave/temp locations, absence of a save operation in the adapter, process disposal, and process/session-level evidence such as the correction adapter/module identity plus bpy.data.filepath before and after execution. Filesystem comparison alone is insufficient.
...
The C++ implementation must also consume the canonical postcondition binding generated by the Atlas correction layer; it must not select its own predicate/version.

## 15. Required adversarial matrix

Request/plan integrity: unknown operation, malformed envelope, missing field, extra field, invalid digest, plan mismatch, correction mismatch, source mismatch, postcondition reference mismatch, caller-supplied postcondition definition/digest mismatch.
...
Persistence: attempted save, unexpected output file, unexpected .blend/.blend1 or autosave artifact, source overwrite attempt, accidental use of persisted-state evidence in place of same-session in-memory evidence.
...
2. operation allowlist and explicit refusal of future/unimplemented correction types;
3. canonical postcondition binding cannot be caller-selected;
4. same-session lifecycle;
5. no-save behavior, including load-time/autosave/file-inventory evidence and process/session-level evidence;
6. adapter isolation;
7. real extraction composition;
8. partial/ambiguous failure disposal;
9. correction receipt preservation;
10. generic-stack non-regression.
...
- no-save audit is clean, including load-time/autosave evidence and process/session-level proof;
...
- no caller-controlled postcondition authority exists;
