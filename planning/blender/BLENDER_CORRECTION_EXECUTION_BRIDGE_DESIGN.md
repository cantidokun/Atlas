...
4. Canonically compute/verify the expected postcondition binding; caller-controlled predicate definitions are rejected.
5. Open the bounded Blender session without persistence.
6. Extract authoritative pre-state.
7. Run existing correction preconditions.
8. For W2/W3, verify the existing authorization artifact through correction_authorization.
9. Translate the already-approved correction into one bounded engine operation.
10. Execute exactly one intended correction mutation.
11. Return a typed mutation result.
12. Freshly extract state in the same session.
13. Run existing canonical postconditions.
14. Emit the existing canonical correction result and receipt.
15. Dispose the session.
16. Persist only if an explicit outer workflow later elects to do so.
