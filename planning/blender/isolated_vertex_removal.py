...
implicit vertex selection is permitted here.
"""
...
class IsolatedVertexPlan:
    ...
    mesh_id: str
    expected_isolated_vertex_indices: Tuple[int, ...]
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "params": {"expected_isolated_vertex_indices": list(self.expected_isolated_vertex_indices)},
...
def _validate_isolated_indices(indices: Any) -> Tuple[int, ...]:
    if type(indices) not in (tuple, list):
        raise IsolatedVertexRemovalError("expected_isolated_vertex_indices must be a tuple/list", "INDICES_TYPE_INVALID")
    if any(type(i) is not int or isinstance(i, bool) for i in indices):
        raise IsolatedVertexRemovalError("isolated vertex indices must be exact integers", "INDEX_TYPE_INVALID")
    result = tuple(indices)
    if any(i < 0 for i in result):
        raise IsolatedVertexRemovalError("isolated vertex indices must be non-negative", "INDEX_NEGATIVE")
    if tuple(sorted(set(result))) != result:
        raise IsolatedVertexRemovalError("isolated vertex indices must be sorted and unique", "INDEX_ORDER_INVALID")
    return result
...
def _plan_body(*, target_object_id: str, mesh_id: str, expected: Tuple[int, ...], source_digest: str) -> dict:
    correction_id = _digest({
        ...
        "mesh_id": mesh_id,
        "expected_isolated_vertex_indices": list(expected),
    ...
    body = {
        ...
        "target_object_id": target_object_id,
        "mesh_id": mesh_id,
        "params": {"expected_isolated_vertex_indices": list(expected)},
    }
    return {**body, "plan_id": _digest(body)}


def plan_isolated_vertex_removal(
    scene: SceneModel,
    source_report_digest: str,
    *,
    target_object_id: str,
    expected_isolated_vertex_indices: Any,
) -> Mapping[str, Any]:
    if type(source_report_digest) is not str or len(source_report_digest) != 64:
        raise IsolatedVertexRemovalError("source report digest must be a 64-character string", "SOURCE_DIGEST_INVALID")
    if type(target_object_id) is not str or not target_object_id:
        raise IsolatedVertexRemovalError("target object id is required", "TARGET_OBJECT_INVALID")
    expected = _validate_isolated_indices(expected_isolated_vertex_indices)
    if not expected:
        raise IsolatedVertexRemovalError("Wave 6 requires at least one isolated vertex", "EMPTY_REMOVAL")
    target = _find_object(scene, target_object_id)
    if target is None or target.mesh is None:
        raise IsolatedVertexRemovalError("target object must resolve to one mesh", "TARGET_NOT_FOUND")
    actual = _isolated_indices(target.mesh)
    if actual != expected:
        raise IsolatedVertexRemovalError("authorized set must exactly equal current isolated set", "ISOLATED_SET_MISMATCH")
    return _plan_body(
...
def _remap_mesh(mesh: MeshModel, removed: Tuple[int, ...]) -> MeshModel:
    ...
    survivors = tuple(i for i in range(len(mesh.vertices)) if i not in removed_set)
    mapping = {old: new for new, old in enumerate(survivors)}
    new_faces = tuple(tuple(mapping[idx] for idx in face) for face in mesh.faces)
    return MeshModel(
        mesh_id=mesh.mesh_id,
        vertices=tuple(mesh.vertices[i] for i in survivors),
        faces=new_faces,
...
def execute_remove_isolated_vertices(
    ...
    *,
    ...
    try:
        expected = _validate_isolated_indices(params["expected_isolated_vertex_indices"])
    except IsolatedVertexRemovalError as exc:
        return ExecutionOutcome(False, "PLAN_INVALID", exc.code, str(source), None)
    if not expected:
        return ExecutionOutcome(False, "PLAN_INVALID", "EMPTY_REMOVAL", str(source), None)
    ...
    if _isolated_indices(after_target.mesh):
        return ExecutionOutcome(False, "POSTCONDITION_FAILED", "ISOLATED_VERTICES_REMAIN", fresh_digest, None)
    ...
    output_digest = _digest(after_target.mesh.to_dict() if hasattr(after_target.mesh, "to_dict") else {
        "mesh_id": after_target.mesh.mesh_id, "vertices": after_target.mesh.vertices, "faces": after_target.mesh.faces,
    })
