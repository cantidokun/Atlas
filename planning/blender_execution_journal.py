...
class SQLiteBlenderExecutionJournal:
    ...
    def __init__(self, database_path: str) -> None:
        ...
        self._connection.execute(
            ...
            "arguments_digest TEXT NOT NULL, status TEXT NOT NULL, "
    ...
    def begin(self, action: ActionSpec, authorization: BlenderWriteAuthorization) -> bool:
        """Record an execution attempt; return False when the authorization is already journaled."""
        ...
        with self._lock:
            cursor = self._connection.execute(
                ...
                "(authorization_id, tool, arguments_digest, status, started_at) VALUES (?, ?, ?, ?, ?)",
                (
    ...
    def complete(
        ...
        with self._lock:
            self._connection.execute(
                "UPDATE blender_execution_journal SET status = ?, completed_at = ?, "
                "receipt_digest = ?, outcome_status = ?, error_type = ? WHERE authorization_id = ?",
                (
                    "COMPLETED",
                    datetime.now(timezone.utc).isoformat(),
                    None if receipt is None else receipt.result_digest,
                    outcome_status,
                    error_type,
                    authorization.authorization_id,
                ),
    ...
    def get(self, authorization_id: str) -> Optional[Mapping[str, Any]]:
        ...
        with self._lock:
            row = self._connection.execute(
                "SELECT authorization_id, tool, arguments_digest, status, started_at, "
                "completed_at, receipt_digest, outcome_status, error_type "
                "FROM blender_execution_journal WHERE authorization_id = ?",
                (authorization_id.strip(),),
            ).fetchone()
        if row is None:
            return None
        keys = ("authorization_id", "tool", "arguments_digest", "status", "started_at", "completed_at", "receipt_digest", "outcome_status", "error_type")
