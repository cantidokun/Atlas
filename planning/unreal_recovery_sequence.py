...
def build_reassessment_plan(plan: UnrealTaskPlan, failure: UnrealPlanExecutionFailure) -> UnrealTaskPlan:
    ...
    for _, operation, _, inspect_name, _, _ in _write_steps(plan, failure):
        ...
        operations.append(UnrealOperation(capability=inspection_capability, kind=UnrealOperationKind.READ, name=inspect_name, arguments={"entity_ids": tuple(operation.entity_ids)}, entity_ids=tuple(operation.entity_ids)))
...
def _verify(step, evidence):
    ...
    if operation.name == "set_actor_location": verify_actor_location(evidence, expected)
    elif operation.name == "set_actor_rotation": verify_actor_rotation(evidence, expected)
    elif operation.name == "set_actor_scale": verify_actor_scale(evidence, expected)
    elif operation.name == "apply_material_variant": verify_material_variant(evidence, expected)
    elif operation.name == "apply_niagara_variant": verify_niagara_variant(evidence, expected)
    else: raise ValueError(f"unsupported recovery verifier for '{operation.name}'")
...
def assess_reassessment_sequence(plan: UnrealTaskPlan, failure: UnrealPlanExecutionFailure, result: UnrealPlanExecutionResult) -> UnrealRecoverySequenceAssessment:
    ...
    if not isinstance(result, UnrealPlanExecutionResult): raise TypeError("result must be a UnrealPlanExecutionResult instance")
    ...
    for step in steps:
        ...
        if not evidence_list:
            ...
            continue
        try: _verify(step, evidence_list[-1])
        except (TypeError, ValueError): assessments.append(UnrealRecoveryStepAssessment(index, operation.name, tuple(operation.entity_ids), "replacement_required", "fresh Unreal state does not match the requested state"))
        else: assessments.append(UnrealRecoveryStepAssessment(index, operation.name, tuple(operation.entity_ids), "already_applied", "fresh Unreal state matches the requested state"))
...
def build_replacement_plan(plan: UnrealTaskPlan, assessment: UnrealRecoverySequenceAssessment) -> UnrealTaskPlan:
    ...
    if not isinstance(assessment, UnrealRecoverySequenceAssessment): raise TypeError("assessment must be a UnrealRecoverySequenceAssessment instance")
    if any(step.disposition not in {"already_applied", "replacement_required", "manual_review"} for step in assessment.steps): raise ValueError("assessment contains an invalid recovery disposition")
    seen_indices = set()
    for step in assessment.steps:
        if step.operation_index in seen_indices: raise ValueError("assessment contains duplicate operation indices")
        seen_indices.add(step.operation_index)
        if step.operation_index < 0 or step.operation_index >= len(plan.operations): raise ValueError("assessment operation index is outside the source plan")
        source_operation = plan.operations[step.operation_index]
        if source_operation.name != step.operation_name: raise ValueError("assessment operation name does not match the source plan")
        if tuple(source_operation.entity_ids) != tuple(step.entity_ids): raise ValueError("assessment entity_ids do not match the source plan")
    if any(step.disposition == "manual_review" for step in assessment.steps): raise ValueError("replacement plan cannot be built while any recovery step requires manual review")
    replacement_indices = {step.operation_index for step in assessment.steps if step.disposition == "replacement_required"}
    if not replacement_indices: raise ValueError("replacement plan requires at least one replacement_required step")
    operations = []
    for index in sorted(replacement_indices):
        operation = plan.operations[index]
        if operation.kind is not UnrealOperationKind.WRITE or operation.name not in _WRITE_DEFINITIONS: raise ValueError("recovery replacement assessment must reference supported write operations")
        ...
        if operation.name in {"set_actor_location", "set_actor_rotation", "set_actor_scale"}:
            ...
            verify_args["expected_material_variant"] = dict(operation.arguments[argument_key])
        elif operation.name == "apply_niagara_variant":
            verify_args["expected_niagara_variant"] = dict(operation.arguments[argument_key])
        operations.extend((
...
def execute_replacement_authorized(executor: UnrealPlanExecutor, replacement_plan: UnrealTaskPlan, authorization: UnrealPlanAuthorization) -> UnrealPlanExecutionResult:
    if not isinstance(executor, UnrealPlanExecutor): raise TypeError("executor must be a UnrealPlanExecutor instance")
...
def execute_recovery_sequence(executor: UnrealPlanExecutor, plan: UnrealTaskPlan, failure: UnrealPlanExecutionFailure, reassessment_authorization: UnrealPlanAuthorization, replacement_authorization: Optional[UnrealPlanAuthorization] = None) -> UnrealRecoverySequenceResult:
    ...
    if not isinstance(failure, UnrealPlanExecutionFailure): raise TypeError("failure must be a UnrealPlanExecutionFailure instance")
    if not isinstance(reassessment_authorization, UnrealPlanAuthorization): raise TypeError("reassessment_authorization must be a UnrealPlanAuthorization instance")
    if replacement_authorization is not None and not isinstance(replacement_authorization, UnrealPlanAuthorization): raise TypeError("replacement_authorization must be a UnrealPlanAuthorization instance or None")
    ...
    if assessment.disposition == "manual_review":
        if replacement_authorization is not None: raise ValueError("replacement authorization must not be supplied for manual review")
        return UnrealRecoverySequenceResult(reassessment_result, assessment)
    if assessment.disposition == "already_applied":
        if replacement_authorization is not None: raise ValueError("replacement authorization must not be supplied when recovery is already applied")
        return UnrealRecoverySequenceResult(reassessment_result, assessment)
    replacement_plan = build_replacement_plan(plan, assessment)
    if replacement_authorization is None: raise ValueError("replacement_required recovery requires a separate replacement authorization")
